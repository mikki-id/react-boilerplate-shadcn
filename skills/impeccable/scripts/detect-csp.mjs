...
const SCAN_EXTS = new Set(['.js', '.mjs', '.cjs', '.js', '.mts', '.cts', '.js', '.jsx']);
const LAYOUT_EXTS = new Set(['.js', '.jsx', '.astro', '.vue', '.svelte', '.html']);
const MAX_DEPTH = 6;
...
export function detectCsp(cwd = process.cwd()) {
  ...
  walk(cwd, cwd, 0, (absPath, relPath, body) => {
    ...
    if ((base === 'middleware.js' || base === 'middleware.js' || base === 'middleware.mjs') &&
